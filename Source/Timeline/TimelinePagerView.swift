...
public protocol TimelinePagerViewDelegate: AnyObject {
  ...
  func timelinePager(timelinePager: TimelinePagerView, didTapAt date: Date)
...
public class TimelinePagerView: UIView, UIGestureRecognizerDelegate, UIScrollViewDelegate, DayViewStateUpdating, UIPageViewControllerDataSource, UIPageViewControllerDelegate, TimelineViewDelegate, EventViewDelegate {
  ...
  @objc func handlePanGesture(_ sender: UIPanGestureRecognizer) {
    ...
    if let pendingEvent = pendingEvent {
      ...
      print(" diff \(diff)")
      var newFrame = pendingEvent.frame
      newFrame.origin.x += diff.x
      newFrame.size.height += diff.y
      
      pendingEvent.frame = newFrame
      prevOffset = newCoord
      accentDateForPendingEvent()
    }
  ...
  public func timelineView(_ timelineView: TimelineView, didTapAt date: Date) {
    delegate?.timelinePager(timelinePager: self, didTapAt: date)
  }
